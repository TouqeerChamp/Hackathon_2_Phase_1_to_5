...
class TodoAgent:
    ...
    def parse_command(self, command: str) -> Tuple[str, list]:
        ...
        # Pattern for "add [title]" or "add task [title]"
        # Captures everything after "add" or "add task" as the title
        add_pattern = r'^add\s+(?:task\s+)?(.+)$'
        add_match = re.match(add_pattern, command)
        if add_match:
            title = add_match.group(1).strip()
            description = ""  # No description support via agent commands
